...
// ── Surface acceptance: bounded narrow layouts, no emoji (#162) ────

{
  // Walk the production manager views — browse, definition actions, the
  // runs entry, the runs list, the Diagnostics view, and the inbox — and
  // assert every rendered line stays inside the width at every display
  // boundary, uses the one-cell ● rail without family icons, and carries
  // no emoji presentation characters anywhere.
  const registry = discoverShadowDefinitions(packageRoot, { projectTrusted: true });
  const cohorts = {
    model: "dddddddddddddddd", thinking: "eeeeeeeeeeeeeeee", toolSchema: "bbbbbbbbbbbbbbbb",
    system: "aaaaaaaaaaaaaaaa", cwd: "ffffffffffffffff", trajectory: "cccccccccccccccc",
  ...
  const result = {
    id: "shr-1", shadowId: "session-synthesizer", shadowName: "Session synthesizer",
    trigger: "manual", source: "automatic", primaryTrigger: "failure", taskIdentity: { epoch: 1 },
    payload: { summary: "Two decisions remain open." }, summary: "Two decisions remain open.",
    delivery: "notified", attention: "unread", createdAt: 2_000,
  };
  const service = makeRuntimeService({
  ...
  const EMOJI = /[\u{1F000}-\u{1FAFF}\u{FE0F}\u{20E3}]/u;
  const checkSurface = (manager, label) => {
    for (const width of [39, 40, 63, 64, 80, 99, 100, 120]) {
      const lines = render(manager, width);
      assert.ok(
        lines.every((line) => line.length <= width),
        `${label}: every line stays inside width ${width}`,
      );
      assert.ok(
        lines.every((line) => !EMOJI.test(line)),
        `${label}: no emoji presentation characters at width ${width}`,
      );
      assert.match(lines[0] ?? "", /●/, `${label}: the one-cell ● rail leads at width ${width}`);
    }
  };

  {
    ...
    checkSurface(manager, "browse");
    manager.handleInput("\r");
    checkSurface(manager, "actions");
    manager.handleInput("escape");
    manager.handleInput("r");
    checkSurface(manager, "runs entry");
    manager.handleInput("\r");
    checkSurface(manager, "runs list");
    manager.handleInput("escape");
    manager.handleInput("r");
    manager.handleInput("down");
    manager.handleInput("down");
    manager.handleInput("\r");
    checkSurface(manager, "diagnostics");
    manager.handleInput("escape");
    manager.handleInput("r");
    manager.handleInput("down");
    manager.handleInput("\r");
    checkSurface(manager, "inbox list");
  }
}
