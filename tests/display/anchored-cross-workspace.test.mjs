...
try {
  const readIn = (workspace) => transformAnchoredReadContent(
  ...
  const replaceIn = (workspace) => createAnchoredReplaceToolDefinition(workspace, undefined, undefined, undefined, false);

  const readA1 = readRows(await readIn(workspaceA));
  const readB1 = readRows(await readIn(workspaceB));
  ...
  await replaceIn(workspaceA).execute(
    "replace-a",
  ...
  const readB2 = readRows(await readIn(workspaceB));
  ...
  await replaceIn(workspaceB).execute(
    "replace-b",
  ...
  const storeA = join(workspaceA, ".pi", "anchored-edit", "hash-store.sqlite");
  const storeB = join(workspaceB, ".pi", "anchored-edit", "hash-store.sqlite");
  ...
  const held = await acquireFileLock(lockFilePath(realpathSync(workspaceA), canonical));
  try {
    const readA2 = readRows(await readIn(workspaceA));
    const oneA2 = readA2.find((row) => row.text === "one");
    assert.ok(oneA2, "workspace A can still read while holding its own lock");
    const blocked = await replaceIn(workspaceA).execute(
      "replace-a-locked",
    ...
    const readB3 = readRows(await readIn(workspaceB));
  ...
  assert.equal(existsSync(lockFilePath(realpathSync(workspaceA), canonical)), false, "the held lock is released");
