...
export function buildScript(scenario, placement) {
  ...
  const maintenanceStep = "Only if a separate current Context Memory maintenance advisory explicitly requests compaction, complete its invited maintenance with compact_to_memory_block as the sole call of its batch, then continue. Without that advisory, do not initiate or retry compression. If it returns SOURCE_NOT_SERVED, do not retry: continue the task and wait for a new advisory.";
  const recoveryStep = definition.requireSourceRead
    ? " Independently, before serializing the handoff, verify every requested fact, including whether any fact remains unknown, from original Memory-source evidence using the available retrieval tools. A summary or assistant copy never authorizes a fact. Search or read directly as needed; no exhaustive paging is required. A complete search means only that its bounded term scan finished, not that a shown excerpt is a complete page or source. Read a referenced page when an excerpt omits any requested value, qualifier, scope, or neighboring context. Reading Memory verifies facts; it is not permission to initiate or retry compression."
    : "";
  return { id: selected.id, placement, setupFiles: { ...definition.files }, introPrompt: place(definition.source, placement), ...(definition.revision ? { revisionPrompt: definition.revision } : {}), ...(definition.abandoned ? { abandonedPrompt: definition.abandoned } : {}), finalPrompt: `Complete the handoff now. ${maintenanceStep}${recoveryStep} Use the native write tool with path handoff.json to write one JSON object with exactly these keys and primitive types (${types}). Use null for unknown facts: unknown means missing knowledge, never the literal string "unknown". Add no keys, arrays, nested objects, commentary, or guesses.`, artifactPath: "handoff.json", oracle: { expected: { ...definition.expected }, critical: [...definition.critical], continuity: [...definition.continuity], unknown: [...definition.unknown], constraints: [...definition.constraints], abandonedValues: [...definition.abandonedValues], requireOriginalEvidence: definition.requireSourceRead === true, evidenceRequirements: (definition.evidenceRequirements ?? []).map((item) => ({ ...item })) }, evidenceTokens: Object.values(definition.expected).filter((value) => typeof value === "string") };
}
