...
describe("edit tool file mutation queue", () => {
  it("uses the same queue key for repeated edits to the same path", async () => {
    await withTempFile("sample.ts", "alpha\nbeta\ngamma\n", async ({ cwd }) => {
      ...
      expect(r1.details?.metrics?.classification).toBe("applied");
      expect(r1.details?.metrics?.added_lines).toBe(1);
        expect(r1.details?.metrics?.removed_lines).toBe(1);
      const r2 = await editTool.execute(
        "e2",
        { path: "sample.ts", remove_from: hashes[1]!, remove_to: hashes[1]!, replacement_text: "BETA" },
        undefined, undefined, ctx,
      );
      expect(r2.details?.metrics?.classification).toBe("applied");
      expect(r2.details?.metrics?.added_lines).toBe(1);
        expect(r2.details?.metrics?.removed_lines).toBe(1);
    });
  });
