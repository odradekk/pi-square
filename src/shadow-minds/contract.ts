...
export function buildShadowDefinitionContract(minimal: EffectiveShadowDefinition): ShadowDefinitionContract {
  ...
  const fields: Record<ShadowDefinitionField, ShadowContractField> = {
    ...
    model: { pattern: bounds.modelReference.pattern.source },
    thinking: { enum: [...SHADOW_THINKING_LEVELS] },
    timeoutSeconds: { min: bounds.runBudgets.min, max: SHADOW_MINDS_RUN_TIMEOUT_HARD_MAX_SECONDS },
    maxTurns: { min: bounds.runBudgets.min, max: SHADOW_MINDS_MODEL_TURNS_HARD_MAX },
    maxToolCalls: { min: bounds.runBudgets.min, max: SHADOW_MINDS_TOOL_CALLS_HARD_MAX },
    tools: {
      maxEntries: bounds.tools.maxEntries,
      unique: true,
      entryPattern: bounds.tools.entryPattern.source,
      default: minimal.tools ?? [...SHADOW_DEFAULT_TOOLS],
      emptyListMeans: "no tools",
      catalogIsFixed: true,
    },
    requiredTools: {
      maxEntries: bounds.requiredTools.maxEntries,
      unique: true,
      entryPattern: bounds.requiredTools.entryPattern.source,
