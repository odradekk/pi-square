...
const SECRET_ASSIGNMENT_PATTERN = /((?:"?(?:authorization|api[_-]?key|token|access[_-]?token|refresh[_-]?token|password|passphrase|client[_-]?secret|private[_-]?key|secret)"?)\s*[=:]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;}]+)/gi;
