...
documented here, and every code documented here is emitted by the module
source.
...
| `[E_FILE_LOCKED]` | A subagent `write` refused: the cross-process write lock on the target file could not be acquired within the bounded wait because another editor holds it. The refusal leaves state untouched for a retry; for `replace` the equivalent lock refusal is `[E_RANGE_STALE]` with fresh anchors. |
