...
| `[E_STATE_UNAVAILABLE]` | The replace's file commit succeeded but recording fresh anchors failed. The file change is reported truthfully, fresh anchors are suppressed, and the message directs a `read` to republish current anchors; stale state cannot authorize another replace until then (#264). |
