...
// pi-square adaptation: pi-square compiles against ES2022, where
// Array.prototype.findLast is not in the TS lib, so the last-mismatch lookup is
// an explicit reverse scan with identical semantics to upstream's findLast.
export function findLastMismatch(
	mismatches: HMismatch[],
	ref: Anchor,
): HMismatch | undefined {
	for (let i = mismatches.length - 1; i >= 0; i--) {
		if (mismatches[i]!.ref === ref) return mismatches[i];
	}
	return undefined;
}
...
export function resolveRange(
	...
	if (!startResolved || !endResolved) {
		if (!startResolved && endResolved) {
			const startMismatch = findLastMismatch(mismatches, edit.hash_bounds[0]);
