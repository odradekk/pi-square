...
| **`/new`**   | Empty the context window and start a fresh session from nothing.   |
| **`/handoff`** | Write a portable markdown file and seed a session anywhere with it. |
| **Subagent** | Delegate the task to its own context window with `subagent_delegate` and get a report back. |
...
**2. Is the context irrelevant to what comes next?** Is everything in this session — the exploration, the decisions, the dead ends — disposable? If so, **`/new`**. It is the cheapest move on the board: it takes no time and hands back the whole window. `/new` also isn't terminal — the old session stays resumable with `/resume`.
...
- swapping to a **new harness** (Pi → Claude Code, Pi → Codex),
