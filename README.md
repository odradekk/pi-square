...
Every anchored operation runs through one per-target operation boundary (#264). Anchored reads hold the target's cross-process exclusion from reading file bytes through committing the matching snapshot and served hashes as one transaction, and every mutation — `replace`, the parent `write`, and a writable subagent's anchored `write` — uses Pi's per-file mutation queue as the outer in-process serializer and the anchored cross-process lock as the inner one, so same-process writer pairs always settle in a well-defined order instead of contending with themselves. The lock files live under `anchored-edit/locks/` beside the initiating session's store and are keyed per operation key — the canonical target, or the file's stable identity when an existing file has multiple hard links, so editing through either name coordinates — with external targets' locks staying attributed to the workspace that initiated the edit, so parallel edits to different files never contend. Lock ownership is a complete atomic record (a random token plus local process identity, with a strictly-numeric process start time) published without a partial-state window. Removal is marker-guarded: a remover first publishes a short-lived per-target removal marker, and only then — with every other remover and every successor excluded — takes and deletes the exact file it re-verified, so a stale verifier arriving after a successor installed touches nothing and the canonical lock path is never emptied behind a live successor (no third writer can slip in, and no residue is left). A marker whose holder died is reclaimed the same positive-death way; a live marker makes the attempt retry within the caller's bounded budget. A lock whose owner is confirmed dead locally is reclaimed, while a live, foreign-host, or malformed lock is only ever waited on — elapsed time alone never proves death. A second Pi session in the same workspace comes under the same discipline: two agents editing the same file produce one success and one recoverable refusal, never a silent overwrite. A bounded wait or cancellation — including an aborted tool call waiting on the lock — ends in `[E_FILE_LOCKED]` (nothing was modified; retry), while `[E_RANGE_STALE]` is reserved for a range that no longer matches the file after the lock was acquired and returns the current range with fresh anchors for the retry. A replace whose file commit succeeded but whose anchor recording failed keeps the truthful success, suppresses fresh anchors, and warns with `[E_STATE_UNAVAILABLE]` to read again before the next edit; an anchored `write` whose state recording fails behaves the same way — the write is reported truthfully with a bounded note directing a fresh read, and the unrecorded state cannot authorize a later replace because the write's state publication is one transaction that either installs the new version's rows or leaves the previous version's rows (stale against the written bytes, refusing every old anchor — unchanged rows included — until a fresh read). When the complete anchored surface is unavailable (another extension owns the built-in `read` or `write`, or the anchor store could not be initialized), the parent write's injected operation performs Pi's plain filesystem write with no anchored lock, store change, or anchor appendix, instead of running half-activated.
