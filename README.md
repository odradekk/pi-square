...
`abort_subagent` validates the complete selection before any abort signal is sent: one malformed, unknown, or foreign ID rejects the whole call, and repeated IDs deduplicate in first-occurrence order. Queued, running, and cancelling targets then receive an abort signal through the same cancellation seam the `/subagent` manager's Cancel action uses, and the tool waits until each active target reaches the `aborted` terminal state. Once a signal has linearized against an active target, abort wins a simultaneous natural-completion race and the run finishes `aborted`; a target that was already terminal before the request keeps its real state and is only reported — completed without repeating its successful result, failed with its complete established bounded error, and aborted with its abort reason.

A successful abort request is a successful tool call even though its active targets end `aborted`; tool-level error is reserved for request validation, ownership conflicts, and infrastructure failures. Interrupting the tool's own wait does not retract abort signals already sent — the targets keep stopping on their own. Aborting never claims or consumes a result: a run claimed by `wait_subagent` stays owned by that waiter, which receives the aborted terminal outcome, and an ordinary aborted run never enters the automatic completion delivery. The versioned abort details preserve request order and record each target's state before the request, its terminal state, whether an abort signal was applied, and its bounded failure or abort reason, using the same bounded projection discipline as wait details.
