...
The shared allowlisted activity formatter renders tool identity plus structurally safe metadata only — URL and query counts, numeric line ranges; every free-form argument value (a path, pattern, query, command, or identifier) is omitted because any model-authored string can carry a credential, and no surface renders tool result payloads. Credential-form redaction stays as defense in depth for the remaining bounded text. Every surface labels runs by shortened IDs: the roster's collision-safe prefixes start at eight characters, extend only while two retained children would collide, and middle-elide under narrow width instead of dropping the lifecycle. Normal results omit system prompts, raw session JSON, and artifact paths. Rendering removes terminal controls and redacts common credential forms without changing model-facing content or background delivery.
