...
Display diagnostics publish through Pi's official `ctx.ui.setStatus()` API, and a conditional third footer row displays them with their own state markers (! for diagnostics, · for other extension statuses) rather than a blanket warning marker. Background subagents no longer publish a footer status: while the current parent session has children visible in the current main task — active children plus those that terminalized inside it — a session-scoped vertical roster appears below the status area and above the editor. The roster lists children only: the main session is never a roster row and stays in Pi's native transcript, and the whole surface — roster, keyboard entry, and overlay — is built on Pi 0.84.2 public extension APIs alone, with no private Pi API and no upstream fork. The roster keeps one row per such child in roster-creation order (full public ID as the deterministic tie-breaker); a resumed public ID keeps its single row and slot, and lifecycle or activity changes never reorder rows. Each row is one physical line showing a hollow selection marker, the role, a collision-safe public-ID prefix (eight characters, extended only while ambiguous within the roster), the lifecycle (`– queued`, `● running`, `× cancelling`, `✓ completed`, `✗ failed`, `× aborted`), the elapsed duration, and a latest-activity summary that carries only tool identity and structurally safe metadata such as counts and line ranges — free-form argument values (paths, patterns, queries, commands, identifiers) never render, and neither do tool-result payloads. Under width pressure a row drops latest activity first, then the duration, then truncates the role while keeping marker, unique ID, and lifecycle. At most ten child rows show at normal terminal heights, the budget shrinks on short terminals, and rows beyond the initial viewport are accounted for with a `… +N more` line. The roster is absent when no child is visible in the current main task and is never created outside interactive sessions; it adds no state or retention beyond the existing background store. Roster visibility follows the current main task: completed, failed, and aborted rows stay inspectable until main accepts the next real prompt, which expires the preceding task's ordinary terminal rows, while queued, running, and cancelling children survive the prompt and stay visible when they later terminalize. The boundary is where main provably accepted the prompt — the agent-run start for an idle prompt, the injected user message for a steer or follow-up queued during a running turn — so a prompt another extension consumes from the input chain, one that fails before the run starts, slash commands, local `!` shell commands, editor drafts, roster navigation, overlay changes, transcript scrolling, and extension follow-ups such as the `/subagent` Config Guide never expire anything. The store's finished-job bound and pending/claimed delivery exemptions stay the single retention authority; the manager still owns historical inspection, and parent replacement, reload, fork, resume, and shutdown close the overlay, clear the roster, and leave aborting children and resetting delivery to the established shutdown path.

While the native editor holds exactly zero content, the roster is keyboard-selectable: the first Down press selects the first child and the first Up press selects the last, movement clamps at both ends and never wraps, and the current candidate carries a solid selection marker while every other row stays hollow. The viewport follows the candidate so children beyond the first ten rows stay reachable, and a scrolled window states its hidden rows with a leading `… +N earlier` line. Enter opens the selected child in one centered, capturing, read-only transcript overlay while the main session keeps running behind it; Enter without a candidate keeps Pi's behavior. Opening a child is never a session switch: the parent stays Pi's one active writable session, a child is not a writable session from the viewer, and no input is ever sent to the child — typing and pasting always target the main editor. Navigation never activates while the editor holds even one character of draft, whitespace, or pasted content, beginning to edit clears an unopened candidate, and while any pi-square-owned modal (the `/subagent`, `/display`, or `/shadow` manager, the ask wizard, or an SSH confirmation) has focus the keys stay with that modal.

The overlay shows the child's role, collision-safe public-ID prefix, lifecycle, duration, and a bounded, incrementally loaded view of the child's complete persisted history of ordered user, assistant, thinking, tool-call, and tool-result entries. History loads on demand from the child's validated native session file: opening reads one bounded tail page, PageUp at the loaded top requests the next bounded older page until the earliest entry of the original delegation and every same-ID resume is reachable, PageDown reloads newer pages evicted from the bounded in-memory window — stitching forward across a record larger than one page just as the older direction does — and Home/End walk toward the file edges in bounded steps. Page boundaries stitch newline-terminated records at the byte level in both directions, so multibyte text is never split or duplicated; every record must carry the minimal native envelope (a non-empty `type` and `id`, unique within the loaded window), and a malformed, invalid, duplicated, oversized, or replaced record fails that one page in its own direction with a bounded retryable error on the edge it belongs to (the older edge suggests PageUp, the newer edge PageDown) while every already validated page stays visible. Each read opens the file once, verifies that the path still names the same regular-file descriptor, and rejects symlinks — even ones pointing inside the artifacts directory — plus every other non-regular file type. The in-memory window never exposes more than 480 projected items or retains more than 64 parsed pages, so metadata-only history is bounded and metadata-only pages compact without displacing the current visible anchor; pages over the bound keep a window into their own parse, and trimming always drops from the end opposite the paging direction so trimmed history stays reachable both ways. The overlay keeps no cache, index, or sidecar beside the native session file and is pi-square's own projection of it — not Pi's private native transcript pipeline. Every text is a display-safe projection before it renders: user and assistant text and thinking pass the shared credential sanitizer and an explicit length budget, while arbitrary provider error diagnostics never cross into the overlay; Pi's public message components see only those bounded copies and fixed failure states. Tool calls use the same roster-grade allowlisted projection as roster rows — cataloged tool identity plus structural counts and line ranges, with free-form paths, patterns, queries, and commands omitted, and raw arguments never rendering. Pi's native assistant grouping stays intact before its tool rows, and a tool result updates the pending call in place, including after either adjacent page is reloaded. The collapsed tool row never shows result evidence; the expand-tools shortcut (below) expands the row to one bounded, credential-sanitized result projection — never a raw or unbounded payload. The shared operational display owns the tool row's canonical title, lifecycle marker and motion (including no-color fallbacks), color, elapsed duration measured from the enclosing session entries, width behavior, muted safe target, and fixed terminal outcome. Unsupported assistant parts remain visible afterward as sanitized generic lines; system prompts, prompt snapshots, session paths, raw JSON envelopes, internal IDs, call IDs, credentials, and unbounded payloads never render. Queued (`Waiting to start`), starting (`Starting…`), empty failed and aborted (with a fixed reason derived only from lifecycle and the closed error-code vocabulary), completed-empty, and read-error states are explicit; a retryable edge error remains visible even when the loaded page projects no transcript rows. Normal terminals size the overlay near 80% width and 75% height centered; small terminals use a near-fullscreen panel with a one-cell margin, and the layout follows terminal resizes while the overlay stays open. Escape first cancels a changed roster candidate (restoring the open child) and only a second Escape closes the overlay, clears the candidate, and returns focus to the main editor; ordinary typed text, completed IME input, or a paste closes the overlay and replays the complete content into the empty editor without submitting it, while Backspace and Delete leave it open. The expand-tools shortcut is consumed by the overlay and toggles only its own tool rows; every other Pi shortcut stays suppressed while the overlay owns input. The roster itself carries no help row — only child rows and their window accounting; the overlay's single status row (below the body) is the one help surface. Viewing is strictly observational: it never pauses, aborts, resumes, claims, consumes, or delivers a child result and never changes the main transcript. While the child runs, the overlay is live: ephemeral view events derived at the child execution boundary stream the in-flight assistant message as an ordered bounded tail below the persisted window (text and thinking parts keep their native interleaving through the same shared projection the persisted view uses), and tool calls render as live operational rows whose terminal state shows immediately on the end event, before the persisted result lands. Delivery is decoupled from the child: an unobserved child retains no view events, publication only enqueues for an open observer, and the default two-stage scheduler gives the child a continuation turn before viewer work. Ordinary updates drain one per tick. Before a queued structural boundary, superseded no-op tool updates are discarded and cumulative assistant deltas reduce to the newest state; the remaining ordered prefix drains through the newest boundary while a 25 ms total flush budget remains, and a slow observer yields the remainder to another tick. Subscriber callbacks run under a real 25 ms JavaScript watchdog that interrupts and evicts a blocked listener; viewer work never runs in the child's event-dispatch stack, and a viewer defect cannot escape into its run record, artifacts, abort signal, or result delivery. The global feed and subscriber counts are hard-bounded, including omission markers, and parent-session replacement installs a new feed generation so late events from an older child cannot enter the new session. A contained viewer failure renders one bounded diagnostic row. Ordinary streaming repaints coalesce to at most one repaint per ~110 ms through the one session-owned repaint timer; message completion, tool start/end, and lifecycle transitions are delivered and repainted at the first structural flush rather than waiting for that window. Completed messages reconcile against the native session file as Pi appends them: immediately before Pi 0.84.2 persists a completed message, its live event captures the current JSONL size, and persisted projections retain their exact line-start byte offsets. A live entry is confirmed only by the persisted occurrence carrying the same bounded content and native timestamp whose line begins exactly at that pre-append floor. Pi persists each message-end before emitting the next, so delayed delivery, a terminal reconcile that loaded the final entry first, a pre-existing identical message, a repeated identical completion, or demand paging can neither duplicate content nor strand it without a lifetime occurrence ledger; missing position evidence fails closed; a message whose persistence never lands keeps its final text visible, and live tool rows shed only when their own persisted call and result rows appear. The live tail is bounded: overflow sheds the oldest entries with one explicit omission state, uses bounded hashed content/call identities to recover dropped message occurrences one-for-one and terminal tool events only after their result appears, and keeps unknown drops visible; it never sheds the newest entry or drops silently. A child that terminalizes while open stays open and shows its final lifecycle and content, and a queued child shows `Waiting to start` even before its session file exists. Live events stay session-scoped: parent session replacement and shutdown clear the feed together with the overlay. The overlay is pi-square's own bounded projection of the child session, rendered through Pi's public message components and pi-square's display adapters rather than Pi's private native transcript pipeline, so exact parity with Pi's private transcript features is not promised: the native main transcript keeps its own search, prompt jump, and image handling, and the overlay provides no transcript search, no prompt jump, no click selection, no arbitrary third-party renderer registration, and no native image parity — unsupported content renders as sanitized bounded fallbacks. Pi 0.84.2 exposes no public focus query, so a third-party capturing overlay opened on top of the editor can still conflict with the roster's Up/Down — a documented limitation of the global input listener; use the `/subagent` manager to resume or cancel children.

While the overlay is open it also carries the cross-child reading experience. Up and Down move a roster candidate anchored on the open child — clamped at both ends, never wrapping — and the solid roster marker follows the tentative candidate while one exists and returns to the open child when it is cancelled; the roster window keeps that focus row visible under its current row budget, including after a height-only resize. The overlay footer names an off-screen candidate's role and unique short ID so selection is never blind while the overlay covers the roster; under width pressure the hints drop first and the role truncates, so the unique ID is the last thing to shrink. Enter re-points the same overlay at the candidate in place, with no stacked overlays and no pass through the main view; Escape first cancels a changed candidate and restores the open child, and only a second Escape closes the overlay and returns to the main editor. Each child independently preserves its loaded history window, scroll position, follow state, new-output notice, and tool-expansion state across direct switches, so comparing parallel children never restarts at the bottom. The first open follows the transcript tail and keeps following sustained live and persisted growth; scrolling upward suspends following (the position is anchored on its transcript entry, so width changes and narrow/wide round trips re-wrap without jumping to another entry), and new live or persisted output below a suspended view sets a visible `new output below` footer state that End clears while resuming follow — End is the resume key, while the mouse wheel and in-window paging never silently re-enable it. The expand-tools shortcut (ctrl+o by default, honored through Pi's effective keybindings) toggles only the open child's tool rows and never the background main transcript; an expanded row reveals its bounded sanitized result projection while the collapsed row stays exactly one line. The full key map: Escape closes (after cancelling a changed candidate), Up/Down move the roster candidate, Enter opens the candidate, PageUp/PageDown/Home/End and the mouse wheel (fullscreen TUI mode, where the terminal defers wheel events to the overlay; inline regular mode keeps the native scrollback — Pi 0.84.2 exposes no public API for extension-visible wheel events there) scroll the transcript, ctrl+o or the remapped expand-tools binding toggles tool-row expansion, and ordinary typed, composed-IME, or pasted text closes the overlay and replays into the main editor without submitting. View-state transitions stay strictly observational — switching, scrolling, expanding, and cancelling never touch a child's lifecycle, result ownership, delivery, waits, aborts, or resume eligibility.
...
Completion notifications use the V5 payload with the current terminal vocabulary; only V5 notifications are generated, confirmed, and rendered. Each result text is bounded at 24,000 characters, and a failure text uses the same bound. An oversized text keeps its head and its tail with a visible `[omitted N characters]` marker, because a subagent report states its conclusion at the end. The pending set holds at most 50 results, results that are still undelivered are never dropped by job compaction, and the `/subagent` manager marks the individual undelivered runs. The pending set lives in the current parent session only: deleting a run's history drops its result, and a result that is still pending when the session ends is not delivered to the next session.
...
The shared allowlisted activity formatter keeps the existing bounded target and argument summaries for the manager and expanded activity views; unknown tools expose only `called`, and those surfaces never render tool-result payloads. The child transcript viewer is the bounded exception described above: only its expanded tool row may show one credential-sanitized, length-limited result projection. The roster applies a stricter projection of the shared timeline: it renders only cataloged tool identity plus URL/query counts or numeric line ranges, and omits every free-form path, pattern, query, command, or identifier. Credential-form redaction remains defense in depth. Every surface labels runs by shortened IDs: the roster's collision-safe prefixes start at eight characters, extend only while two retained children would collide, and elide under narrow width instead of dropping the lifecycle. Normal results omit system prompts, raw session JSON, and artifact paths. Rendering removes terminal controls and redacts common credential forms without changing model-facing content or background delivery.
...
Context Memory keeps one long conversation branch usable with a smaller active context by representing older history as an ordered list of **Memory blocks** — compact Markdown written by the current main agent inside an ordinary user run — followed by the retained working set of recent conversation. A parent-only `compact_to_memory_block` tool is resident while the feature is enabled: a validated call records the new Memory as a versioned Pi custom state entry (Pi's `SessionManager` stays the only session-file writer) and the **next ordinary model request applies it** — the covered original entries leave, one complete carrier message enters exactly once, and the run continues without settle, abort, restart, or any native compaction call. Existing blocks stay byte-identical across appends, source ranges are validated against complete tool batches by call id with the latest user instruction and the most recent completed work retained raw, a parent-only `read_memory_source` tool recovers any block's original transcript in bounded 16 KiB pages, and a parent-only `search_memory_source` tool locates literal terms (case-insensitive OR, optional block scope) across the current Memory's original sources — returning bounded verbatim snippets with directly readable block/page locations and an opaque view token that a follow-up read validates, rejecting stale references with `VIEW_STALE` after any Memory change while search results stay protocol artifacts that never become new original evidence. Maintenance is sustained inside one long task: pressure is judged before every ordinary request from the messages, effective system prompt, and active tool definitions, with a bounded provider residual for comparable request compositions, one pending maintenance request pins the exact sources the advisory invites (later tool work joins only through an explicit re-scope at a served request), repeated refused, invalid, or zero-benefit attempts against one pinned scope suppress the advisory within a bounded budget until real growth or a Memory change re-enables it, and provider usage is bound to the request and Memory version it measured so a pre-compression report never floors post-compression pressure. pi-square's context handler renders each current Memory block as its own ordered text content block — one uniform projection for every model and provider, with no cache markers and no provider branch — and when an upstream transform breaks the message-to-source mapping the custom application refuses instead of guessing; the narrow exception is Pi's omission of a terminal `error` or `aborted` assistant with an empty content array after interruption, while non-empty and partial failed assistants remain strict. Memory lives only inside the Pi session tree on the current branch: there is no second database, background summarizer, or extra model call, and every unsupported case — an unqualified host, an opaque branch, a refused application — leaves Pi native compaction untouched. Recorded Memory survives interruptions and native session branches truthfully: a cancellation before the write records nothing while the same session can still record, a cancellation after recording keeps the real record with its truthful acknowledgement, restarts re-derive the byte-identical carrier and replacement set without replaying unrecorded candidates or advisories, fork, clone, import, and tree navigation stay branch-private with sibling branches isolated both directions, a later native compaction becomes the new baseline without stacking or resurrecting the superseded record, an unanswered protocol call from an interrupted or branch-cut batch never blocks a later append (an answered `read_memory_source` pair leaves the request together with its covered exchange, or stays whole when it trails the working set — never an unpaired half), and disabling or uninstalling makes the raw history model-visible again without touching the session file, with the same Memory deriving again once the feature returns. The native behaviors are pinned by deterministic tests against real Pi sessions with native tree operations at the provider-request exit; the seams a real session cannot reproduce deterministically (injected write failures, restart applied-flag resets, post-switch source revalidation, interrupted reading-batch shapes) are pinned by clearly separated boundary-injected unit cells combined with that native evidence. When rendered Memory rises above half its budget the next maintenance is a suffix rebuild: the shortest newest block suffix is selected so the kept prefix stays within half the budget, every due request serves that suffix's complete original sources raw while its summaries are absent and the prefix carrier stays byte-exact, acceptance records one block spanning the originals plus new eligible history with the replaced blocks' retained exceptions kept raw, and a rebuild whose complete serving cannot fit the window reports an honest scale limit instead of truncating, paging, or deleting anything. The complete Memory body and the tool protocol are additionally verified through Pi's native Anthropic and OpenAI-compatible conversions at the real transport boundary — unmodified production conversion modules behind a loopback capture server prove pairing, exactly-once carriers, replayed thinking signatures, preserved images, and no injected cache markers across mixed batches, failures, and cancellations. Every request exit arbitrates between the three views it may send: the latest recorded Memory projection applies whenever it validates; when that application refuses and the complete baseline still fits, the request goes out unprojected as the safe native fallback with the unrecorded maintenance candidates discarded and Pi native compaction owning the boundary at its own safe edge; and when no validated view fits under Pi's native compaction boundary — a model ignoring advisories, one oversized tool result, or a no-net-benefit scope — the exit issues the public `ctx.abort()` signal instead of sending anything, proven at the real HTTP transport boundary, with recorded Memory and the truthful applied accounting never touched and no automatic continue. The complete guide is [`docs/context-memory.md`](docs/context-memory.md); the architecture decision is recorded in [ADR 0018](docs/adr/0018-context-memory-request-projection.md).

The model is instructed to maintain Memory only when a current advisory invites it, then continue the task. A completed rebuild is not automatically reopened until new replaceable evidence exists, and it must reduce both the served maintenance request and the normal Memory projection. Reading or searching original sources does not authorize a rejected rebuild. Memory retains non-secret conversation facts separately from ordinary workspace files while respecting explicit retention restrictions.

The runtime guarantee ends at pi-square's own `context` handler. Later extension or provider-payload modifications can change the outgoing request; source observation and `applied` status are not final-delivery receipts. Pi remains unmodified. See the [compatibility boundary](docs/context-memory.md#the-compression-cycle) for the scope of validation.
...
`compressionThreshold` is exactly one of `{ "percent": 10–80 }` or `{ "tokens": ≥ 1 }` (integers), and `memoryBudgetPercent` accepts 1–25 as a percent of the current model's full context window; unknown fields and a project layer that declares `contextMemory` are rejected as a whole. The feature activates by capability detection: it runs on any Pi host that exposes the required public interfaces, whatever version that host reports, and `/context` names the running host version when support fails. Inspect it through the `/context` `memory[]` section — states include `active` (distinguishing Memory that is recorded from Memory whose carrier has been applied to a request), `due`, `opaque`, `no-memory`, `unsupported`, `disabled`, and an `ephemeral` marker for in-memory sessions — and read one block with `/context memory <block> [page]`, which performs no model call.

Disclosure: Memory blocks are model-authored conversation text transmitted to the selected provider like any other message (provider switching carries them), and they are **not** encrypted, secret-scanned, redacted, or securely erased — the compression advisory asks the model not to copy credentials, private keys, or access tokens into Memory, but it can persist sensitive text by mistake. Disabling or uninstalling affects future behavior only: existing state entries remain in Pi history as inert records and the covered originals become model-visible again through Pi's native projection. Pi's `SessionManager` stays the only session-file writer — one writer per session file; run parallel work in forked or cloned sessions — and deleting one session file is not universal erasure of forks, exports, scrollback, or provider records. The provider-cache experiment runs real Pi `AgentSession.prompt()` sequences for Sonnet 5, GLM 5.3, and GPT-5.6 Luna concurrently. Each lane loads pi-square, grows its transcript through Pi, executes the real `compact_to_memory_block` tool loop, and records multiple Memory state entries; there is no synthetic Context, payload, nonce, or manually appended message. Cache hit rate uses Pi's own footer formula: `cacheRead / (input + cacheRead + cacheWrite)`. No success-rate, correctness, or cost-efficiency improvement is claimed until the required real-model and provider-cache evidence exists for the exact release commit.
