...
Every anchored operation runs through one per-target operation boundary (#264). Anchored reads hold the target's cross-process exclusion from reading file bytes through committing the matching snapshot and served hashes, and every mutation — `replace`, the parent `write`, and a writable subagent's anchored `write` — uses Pi's per-file mutation queue as the outer in-process serializer and the anchored cross-process lock as the inner one, so same-process writer pairs always settle in a well-defined order instead of contending with themselves. The lock files live under `anchored-edit/locks/` beside the initiating session's store and are keyed per operation key — the canonical target, or the file's stable identity when an existing file has multiple hard links, so editing through either name coordinates — with external targets' locks staying attributed to the workspace that initiated the edit, so parallel edits to different files never contend. Lock ownership is a complete atomic record (a random token plus local process identity); a lock whose owner is confirmed dead locally is reclaimed, while a live, foreign-host, or malformed lock is only ever waited on — elapsed time alone never proves death. A second Pi session in the same workspace comes under the same discipline: two agents editing the same file produce one success and one recoverable refusal, never a silent overwrite. A bounded wait or cancellation ends in `[E_FILE_LOCKED]` (nothing was modified; retry), while `[E_RANGE_STALE]` is reserved for a range that no longer matches the file after the lock was acquired and returns the current range with fresh anchors for the retry. A replace whose file commit succeeded but whose anchor recording failed keeps the truthful success, suppresses fresh anchors, and warns with `[E_STATE_UNAVAILABLE]` to read again before the next edit.
