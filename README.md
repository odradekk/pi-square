...
The `subagent` tool uses a Pi-native, collapsible editorial layout across foreground runs, resumed sessions, background queue results, and background completion notifications. The unframed presentation uses a restrained near-monochrome hierarchy, width-aware title and status columns, subtle section rules, and responsive activity ledgers instead of cards, background fills, emoji, or undifferentiated log rows. Calls identify the agent and user-facing mode, show a bounded three-visual-line task preview, and report explicit model, effort, context, cwd, and custom-instruction overrides without displaying custom system-prompt text.
