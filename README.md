...
`anchoredEditing.enabled` defaults to `true`, making anchored editing the only parent-session editing path: Pi `read` supplies anchors, `replace` and `revert` are active, and Pi `edit` is absent. Set it to `false` in agent configuration to restore Pi's factory-faithful `read` and `edit` tools without changing the on-disk anchor store. `anchoredEditing.autoRead` is agent-only and defaults to `true`; when it is enabled, a successful changed Pi `write` appends a bounded fresh hashline preview, and successful `replace` and `revert` results retain their authoritative anchored diffs. Set it to `false` to suppress those post-edit anchors while retaining anchored reads, replace, revert, and write-state clearing. When anchored editing is enabled in agent configuration, the registered Pi `read` tool adds stable, unique three-character prefixes to workspace text lines after Pi has read the file. The parent-only `replace` tool replaces an inclusive range by its bare start and end anchors, and the parent-only `revert` tool restores the immediately preceding replace for one workspace file. `replace` verifies that every affected row was served and still matches disk state before it writes. It records the original bytes, line-ending style, byte order mark, and anchors before modifying the file; if that persistence fails, the replace is refused, and if the write fails, the prior revert record is restored. `revert` restores the saved state byte for byte, returns an authoritative anchored difference for follow-up work, and refuses a modified or deleted target without overwriting newer content. A successful Pi `write` clears that file's single-level revert history; a failed write does not. Anchor and served-state refusals return fresh anchored feedback as completed warnings; environment failures remain failed results. Pi's built-in `edit` tool is removed from the active parent tools while this setting is enabled and returns on the next disabled session. Consecutive unchanged reads and changes limited to trailing whitespace keep the same prefixes. The feature preserves Pi image attachments, honors `offset` and `limit`, and gives named alternatives for paths outside the workspace, directories, non-text files, and over-limit source files. Its project-scoped snapshots, revert records, and served-hash state use the `parent` owner in `.pi/anchored-edit/hash-store.sqlite`; it is ignored by Git. The feature is unavailable to child sessions. The setting applies on the next session start.
