...
- **Purpose:** A unified local extension package for the Pi coding agent. It provides prompt management, interactive and session tools, local text, file, and structural code search, web, GitHub, and documentation tools, subagents, TUI customizations, a Scheme sandbox, PowerShell execution, skills, and themes.
...
- `src/search/` owns the bounded `rg`, `fd`, and read-only `sg` wrappers, argument construction, process execution, native-binary resolution, structured output, and native presentation. `rg` and `fd` use Git-tracked binaries; `sg` resolves the exact `@ast-grep/cli` platform package without PATH fallback.
