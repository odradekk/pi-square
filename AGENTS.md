...
- Keep `sg` read-only and syntax-oriented: expose only bounded `ast-grep run` pattern/kind search, force streaming JSON and no color, invoke the resolved native package without a shell or PATH lookup, and never admit rewrite, interactive update, scan, LSP, project scaffolding, or raw CLI arguments. Preserve the distinction between AST structure search and type-aware semantic indexing.
...
- Treat `@ast-grep/cli` and its platform packages as security-sensitive native dependencies. Keep the CLI and optional packages on one exact version, verify target coverage and lockfile integrity on upgrades, and preserve direct package resolution without PATH fallback.
