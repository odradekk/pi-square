...
- `src/core/` owns shared configuration, diagnostics, path handling, workspace-bounded PDF input validation, and the cancellable bounded process runner used by local native tools. Strict configuration V2 merges agent-level `config/pi-square.json` with project-level `.pi/config/pi-square.json`; V1 is rejected with a migration diagnostic. SSH profiles are security-sensitive agent-only configuration and are rejected in the project layer rather than merged.
