...
- Feature directories under `src/` own individual tools or UI behavior: `ask-user/`, `banner/`, `github/`, `notifications/`, `prompt-manager/`, `scheme/`, `search/`, `shell/`, `statusline/`, `subagents/`, `time/`, `todo/`, and `web/`. `src/github/` owns the parent-only authenticated GitHub REST client, bounded read-only tools, and native presentation. `src/shell/` owns platform shell selection, PowerShell process execution, bounded streaming output, and shared bash/PowerShell command presentation. `src/subagents/` owns V2 definition overlays, layered prompt compilation and audit manifests, parent-session-owned persisted child execution, portable child tool policy, background delivery, bounded live updates, the native fg/bg/resume presentation, the integrated `/subagent` manager, and the configuration-guide custom message. `src/statusline/` consumes read-only background snapshots to render the bounded second footer row.
...
- Keep GitHub tools read-only, GitHub.com-only, and parent-session-only. Resolve `GITHUB_TOKEN` before `auth.json` `github.key`; never expose either value in content, details, errors, logs, or rendering. Authenticated redirects must remain on `https://api.github.com`, all remote and model-facing data must have explicit budgets, and every remote/local truncation must remain visible. Do not add GitHub tools to `src/tool-catalog.ts` or bundled subagent definitions without a separate authorization decision. Remember that returned private repository content is persisted in Pi's normal session history even though the module creates no additional cache or artifacts.
