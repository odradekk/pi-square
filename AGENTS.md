...
- Feature directories under `src/` own individual tools or UI behavior: `ask-user/`, `banner/`, `codegraph/`, `footer/`, `github/`, `notifications/`, `pdf-search/`, `prompt-manager/`, `scheme/`, `search/`, `shell/`, `ssh/`, `subagents/`, `time/`, `todo/`, and `web/`. `src/footer/` owns the enhanced native-semantic footer snapshot, responsive renderer, lifecycle, and native fallback. `src/github/` owns the authenticated GitHub REST client, bounded read-only tools, and native presentation shared by the parent session and explicitly opted-in child profiles. `src/pdf-search/` owns local PDF.js text extraction, normalized exact and conservative fuzzy page matching, bounded in-memory caching, and native presentation. `src/web/` owns Jina and Context7 integrations plus the parent-only Firecrawl PDF parser, including workspace-bounded page extraction, confirmed upload, bounded HTTP handling, and native presentation. `src/shell/` owns platform shell selection, PowerShell process execution, bounded streaming output, and shared bash/PowerShell command presentation. `src/subagents/` owns V2 definition overlays, layered prompt compilation and audit manifests, parent-session-owned persisted child execution, portable child tool policy, background delivery, bounded live updates, the `subagent_delegate`/`subagent_resume` tools and their native fg/bg/resume presentation, the integrated `/subagent` manager, the configuration-guide custom message, and the bounded background summary published through Pi's native `setStatus` footer row.
- `src/search/` owns the bounded `rg`, `fd`, and read-only `sg` wrappers, argument construction, native-binary resolution, structured output, and native presentation. `rg` and `fd` use Git-tracked binaries; `sg` resolves the exact `@ast-grep/cli` platform package without PATH fallback.
- `src/codegraph/` owns the local semantic `codegraph` tool, exact platform-bundle resolution, workspace path boundary, index-health and lifecycle policy, bounded execution, and native presentation. The parent definition exposes `explore`, `status`, confirmed `init`, automatic incremental `sync`, and confirmed `reindex`; child definitions are restricted to `explore` and `status`.
- `src/ssh/` owns the parent-only persistent SSH tool, exact `ssh2` client integration, pinned host verification, agent/private-key authentication, masked secret input, guarded connection-event lifecycle, bounded raw output cursors, single-line terminal projection, connection/session limits, idle cleanup, lifecycle teardown, and native presentation. It supports one POSIX foreground command per shell and no child exposure, arbitrary targets, password parameters, agent forwarding, output artifacts, transparent reconnect, SFTP, jump hosts, proxies, or port forwarding.
