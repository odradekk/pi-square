...
- Feature directories under `src/` own individual tools or UI behavior: `ask-user/`, `banner/`, `codegraph/`, `footer/`, `github/`, `notifications/`, `prompt-manager/`, `scheme/`, `search/`, `shell/`, `subagents/`, `time/`, `todo/`, and `web/`. `src/footer/` owns the enhanced native-semantic footer snapshot, responsive renderer, lifecycle, and native fallback. `src/github/` owns the authenticated GitHub REST client, bounded read-only tools, and native presentation shared by the parent session and explicitly opted-in child profiles. `src/shell/` owns platform shell selection, PowerShell process execution, bounded streaming output, and shared bash/PowerShell command presentation. `src/subagents/` owns V2 definition overlays, layered prompt compilation and audit manifests, parent-session-owned persisted child execution, portable child tool policy, background delivery, bounded live updates, the native fg/bg/resume presentation, the integrated `/subagent` manager, the configuration-guide custom message, and the bounded background summary published through Pi's native `setStatus` footer row.
