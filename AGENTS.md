...
- Keep subagent presentation and manager metadata privacy-preserving: render from bounded v3 details without exposing prompt snapshots, custom system text, raw session data, or normal-run artifact paths. The visual language is an unframed responsive editorial layout with restrained semantic color, text-style status glyphs, label-led section rules, and width-aware activity/footer alignment; ACTIVITY and statusline rows show sanitized tool calls and formatted arguments but never tool result payloads. Do not introduce emoji presentation characters, background cards, or undifferentiated log rows.
- Scope `/subagent` lifecycle management to the current parent session. Foreground cancellation stays with Pi's native turn cancellation; queued/background manager cancellation must pass through `cancelling` before terminal `aborted`, retain resumable artifacts, and require separate confirmation for history deletion.
