...
- Keep every model-callable tool parameter schema provider-compatible: use a strict top-level `Type.Object` with `additionalProperties: false`; never use top-level `Type.Union`, `anyOf`, or `oneOf`. Represent operation or mode branches with a required `StringEnum`, enforce branch-specific field combinations at execution validation, and add contract coverage for the top-level object shape, absent unions, and required discriminator.
