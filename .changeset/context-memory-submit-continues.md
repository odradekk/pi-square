...
- The bound on post-submission work is documented and covered: the due point sits a fixed margin (at least ten percent of the model window) below Pi's native compaction boundary, and a run that exhausts that margin before settling meets the existing safe fallback — with a pending candidate Pi's seam consumes it and the Memory compaction commits; with no candidate, Pi native compaction proceeds and its foreign entry closes the due run.
