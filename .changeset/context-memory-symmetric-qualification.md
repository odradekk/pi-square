...
- Replace the historical asymmetric 16-cell continuity matrix with one shared 12-case corpus run by both Sonnet 5 and GLM 5.3 as two concurrently started, internally sequential queues. The v3 report records all 24 isolated cells, 12 matched pairs with nullable cache/retrieval/compression deltas, per-model totals, honest partial/error states, positive cache observations without inventing zero-field presence, retrieval counts and bytes, and bounded private evidence; the v2 reader labels old evidence historical and never treats it as current qualification.
- Credit original source recovery only when an exact successful search/read result is present at the native request exit before the later handoff write and the credited fact shares one returned unit with its complete original provenance. Deterministic native tests reject failed, stale, wrong-view, filtered, clipped, copied, boundary-joined, same-batch, and post-handoff evidence while accepting sufficient search snippets and search-to-targeted-read completion; the final artifact uses the observable native write route.
